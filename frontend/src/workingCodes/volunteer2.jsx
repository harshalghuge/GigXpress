...
const VolunteerDashboard = () => {
  ...
  // ======== Jobs visible to volunteer (posted by organizer) ========
  const [availableJobs, setAvailableJobs] = useState([]);

  // ======== Volunteer applied jobs list ========
  ...
  // ======== Load jobs + applications from localStorage ========
  useEffect(() => {
    // jobs
  ...
  const handleApplySubmit = (formData) => {
    ...
    // prevent duplicate apply (same job + same phone)
    const already = apps.some(
      (a) => a.jobId === selectedJob.id && a.phone === formData.phone
    );
    if (already) {
      alert("You already applied for this job with this phone number.");
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex">
        ...
        <aside
          ...
          } lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-30 w-64 bg-white border-r transition-transform duration-300 ease-in-out shadow-xl lg:shadow-none`}
        >
...
/* ------------------ Local Apply Modal Component ------------------ */
const LocalApplyModal = ({ isOpen, onClose, selectedJob, onSubmit }) => {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-4">
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <button
            type="submit"
            className="w-full px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold hover:brightness-110 hover:shadow-lg transition-all active:scale-95"
          >
            Submit Application
          </button>
        </form>
